...
const NavBar = () => {
    ...
    return (
        <div className="flex justify-between items-center w-full h-20 text-black fixed bg-[#F6F6E9]">
            <div className="flex items-center">
                <img src={Logo} alt="logo" className="h-12 md:h-16 lg:h-20 w-auto ml-4 md:ml-0 max-w-[200px]" />
            </div>

            <ul className="hidden md:flex">
                {links.map(({ id, link }) => (
                    <li key={id} className="px-4 cursor-pointer capitalize font-bold text-black relative group">
